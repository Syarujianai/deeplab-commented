...
def _process_batch(sess, original_images, semantic_predictions, image_names,
  ...
  num_image = semantic_predictions.shape[0] # semantic_predictions.shape[0] = batch.
...
def main(unused_argv):
  ...
  with g.as_default():
    ...
    if FLAGS.min_resize_value and FLAGS.max_resize_value:
      ...
      # syaru: tf.squeeze(): Removes dimensions of `size 1` from the shape of a tensor.
      # "image after tf.squeeze is [height, width, 3]" (drop out the `batch` dimension? Maybe orignally `[batch, height, width, 3]`).
      original_image = tf.squeeze(samples[common.ORIGINAL_IMAGE]) # syaru: ORIGINAL_IMAGE = 'original_image'.
      original_image_shape = tf.shape(original_image)
      # syaru: `tf.slice(inputs,begin,size,name='')`
      ...
      # syaru: `tf.expand_dims(input, axis=None, name=None, dim=None)`
      # 在第axis位置增加一个维度.
      # `tf.image.resize_images(image, (w， h), method)`
      # 第一个参数表示需要调整的图像，第二个参数为调整后图像的大小，第三个参数为插值方法.
    ...
    while (FLAGS.max_number_of_iterations <= 0 or
      ...
      num_iters += 1
      # syaru: `last_checkpoint`: 最后获取的checkpoint file的名称
